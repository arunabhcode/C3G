...
This document describes how to train C3G-F using point-prompted SAM segmentation loss on the Replica SemSeg dataset.
...
The Replica SemSeg dataset should be available at the configured root path. The default config expects it at `/home/arunabh/replica_semseg`. Override via the command line if your path differs.
