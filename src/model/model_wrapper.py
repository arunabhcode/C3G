...
<<<<<<< HEAD
    ...
    @contextmanager
    def _trainable_autocast(self):
        with cuda_autocast(
            enabled=self._bf16_autocast_enabled,
            dtype=torch.bfloat16,
        ):
            yield

    @contextmanager
    def _frozen_autocast(self):
        with frozen_cuda_autocast(
            enabled=self._bf16_autocast_enabled,
            dtype=torch.bfloat16,
        ):
            yield
    ...
    def on_fit_start(self) -> None:
        ...
        requested = self.train_cfg.mixed_precision
        active_dtype = autocast_dtype_name(self._bf16_autocast_enabled)
        if requested == "bf16" and not self._bf16_autocast_enabled:
            reason = (
                "CUDA unavailable"
                if self.device.type != "cuda"
                else "bf16 not supported on this GPU"
            )
            print(
                f"[mixed_precision] requested bf16 but using fp32 ({reason})",
                flush=True,
            )
        else:
            print(
                f"[mixed_precision] enabled={self._bf16_autocast_enabled}, "
                f"config={requested}, autocast_dtype={active_dtype}, "
...
>>>>>>> parent of ae52758 (added mixed precision for decreased RAM for training)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.train_cfg.feature_rendering_loss > 0:
            ...
            feature = self.forward_foundation_model(
                torch.cat(
