...
Vanilla SAM (Replica + ScanNet test)::
    ...
    modal deploy src/modal_infra/modal_eval_masks.py  # HTTP /predict on VanillaSAMService
...
Smoke::
    ...
    modal run src/modal_infra/modal_eval_masks.py::c3g_smoke --wait
...
def vanilla(
    ...
    """Full vanilla SAM mask export, or single-image predict (bytes sent to Modal)."""
    ...
    _dispatch_vanilla(
        "export_masks",
        job_name="vanilla SAM mask export",
        detach=use_detach,
...
def c3gsam_nomaghead(
    ...
    dispatch_remote(
        ...
        eval_output_mount=str(C3G_SAM_NOMAGHEAD_EVAL_OUTPUT_MOUNT),
        mask_batch_size=mask_batch_size,
        with_lightning_test=with_lightning_test,
        detach=resolve_detach(detach=detach, remote_job=not wait),
